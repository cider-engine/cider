        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            var xmlnsContext = context.CompilationProvider
                .Select(static (x, token) =>
                {
                    ...
                    {
                        if (SymbolEqualityComparer.Default.Equals(x.Assembly, assembly)) continue; // 只处理类库里的
                        ProcessAssembly(assembly, mappings, token);
                    }

                    return new AlwaysEqualWrapper<Dictionary<string, Dictionary<string, string>>>(mappings);
                });

            static void ProcessAssembly(IAssemblySymbol assembly, Dictionary<string, Dictionary<string, string>> mappings, CancellationToken token)
            {
                ...
                foreach (var (xmlns, ns) in assembly.GetAttributes()
                    .Where(static attr => attr.AttributeClass.GetFullyQualifiedName() == "global::Cider.Attributes.XmlnsDefinitionAttribute")
                ...
                if (namespaces.Count == 0 && hasNoDirectType) return;

                //foreach (var ns in assembly.GlobalNamespace.GetNamespaceMembers()) ProcessNamespace(ns, namespaces);
                ProcessNamespace(assembly.GlobalNamespace, namespaces, token); // 允许不在命名空间中声明类

                static void ProcessNamespace(INamespaceSymbol @namespace, Dictionary<string, Dictionary<string, string>> namespaces, CancellationToken token)
                {
                    ...
                    foreach (var ns in @namespace.GetNamespaceMembers()) ProcessNamespace(ns, namespaces, token);
                }
            }
